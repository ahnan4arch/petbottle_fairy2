# rge_export2.7.2C.py (c) 2010-2012, Yuhei Akamine
#
...
class tri_wrapper(object):
    ...
    __slots__ = "vi", "mat", "faceuvs", "smooth"

    def __init__(self, vindex=(0, 0, 0), mat=None, faceuvs=None, smooth=False):
        self.vi = vindex
        self.mat = mat
        self.faceuvs = faceuvs
        self.smooth = smooth  # offset indices
...
#                file.write("%s " % mesh.materials[face.material_index].name)
    ...
    def extract_triangles(self, mesh):
        ...
        for i, face in enumerate(mesh.tessfaces):
            ...
            # if f_v[3] == 0:
            if len(f_v) == 3:
                new_tri = tri_wrapper((f_v[0], f_v[1], f_v[2]), face.material_index, us)
                if (do_uv):
                ...
                new_tri = tri_wrapper((f_v[0], f_v[1], f_v[2]), face.material_index, us)
                new_tri_2 = tri_wrapper((f_v[0], f_v[2], f_v[3]), face.material_index, us)
    ...
    def writeMesh(self, mesh):
        ...
        tri_list = self.extract_triangles(mesh)
...
#        print(len(tri_list)) 
        ...
        for v in mesh.vertices:
            file.write(" %0.4f %0.4f %0.4f" % (v.co[0],v.co[1],v.co[2]))
        file.write("\n");
        file.write("    Nor:");
        for v in mesh.vertices:
            file.write(" %0.4f %0.4f %0.4f" % (v.normal[0],v.normal[1],v.normal[2]))
        file.write("\n");
        ...
        file.write("    Tri: "+type);
        for t in tri_list:
            file.write(" %d %d %d %d %d" %(t.vi[0], t.vi[1], t.vi[2], t.mat, t.smooth))
            if do_uv :
                if isinstance(t.faceuvs, tuple):                
                    for i in range(0,3):
                        file.write(" %0.4f %0.4f" % (t.faceuvs[i][0], t.faceuvs[i][1]))
                else:
...
#                print(tex.type)
    ...
    def writeScene(self, filename, scene): 
        ...
        f.write('%Blender RGE exporter 2.7.2\n')
